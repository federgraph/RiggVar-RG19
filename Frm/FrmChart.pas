...
type
  ...
  TChartForm = class(TForm)
    ...
    procedure KurvenZahlSpinnerChanging(Sender: TObject;
...
var
  j, p: Integer;
begin
  if not FBuissy then begin { Berechnen }
    FBuissy := True;
    if not CheckBeforeCalc then Exit;
    { Parameterzahl bearbeiten }
    if PComboBox.Text = ('kein Parameter') then begin
      PMinEdit.Text := '0';
      PMaxEdit.Text := '0';
      KurvenZahlSpinner.Position := 1;
    end;
    ParamCount := KurvenZahlSpinner.Position;
    if PSpinner.Position > ParamCount then PSpinner.Position := ParamCount;
    PSpinner.Max := ParamCount;
      {MaxValue muß größer MinValue sein!}
    if PSpinner.Max = 1 then PSpinner.Max := 2;
    PSpinner.Update;

    if YComboBox.ItemIndex > ANr-1 then YComboBox.ItemIndex := 1;

    ProgressDlg.ShowModal;
    {Calc(Self);} {von ProgressDlg aufgerufen}
    {ProgressDlg.Hide;} {Dialog verschwindet von selbst}
    if (ProgressDlg.ModalResult = mrOK) or ProgressDlg.Aborted then begin
    ...
    for p := 0 to ParamCount-1 do bf[p] := af[p,j];
    UpdateYMinMax;
    Draw;
...
procedure TChartForm.Calc(Sender: TObject); {wird vom Modal angezeigten ProgressDlg aufgerufen}
begin
  UpdateYAchseSet;
  GetCurves;
end;
...
begin
  ...
  for p := 0 to ParamCount-1 do begin
    with ProgressDlg.ParamLabel do begin
    ...
    {Parameter ansteuern}
    S := PComBoBox.Text;
    if ParamCount < 2 then begin {do nothing} end
    ...
    for i := 0 to 100 do begin
      if i mod 5 = 0 then ProgressDlg.Gauge.Position := i;
      ...
      if SalingTyp = stFest then
        if (XComboBox.Text = 'Winkel') or
           (PComboBox.Text = 'Winkel') then Rigg.UpdateGetriebeFS
        else Rigg.BerechneWinkel
      else Rigg.UpdateGetriebe;
      Rigg.UpdateRigg;
      PunktOK := Rigg.GetriebeOK and Rigg.MastOK and Rigg.RiggOK;

      { Ergebnisse einspeichern }
      if yavVorstagSpannung in YAchseSet then begin
        j := YAchseRecordList[yavVorstagSpannung].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[14]
        else af[p,j,i] := 0;
      end;
      if yavWantenSpannung in YAchseSet then begin
        j := YAchseRecordList[yavWantenSpannung].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[8]
        else af[p,j,i] := 0;
      end;
      if yavMastfallF0F in YAchseSet then begin
        j := YAchseRecordList[yavMastfallF0F].ArrayIndex;
        af[p,j,i] := Abstand(Rigg.rP[ooF0],Rigg.rP[ooF]);
      end;
      if yavMastfallF0C in YAchseSet then begin
        j := YAchseRecordList[yavMastfallF0C].ArrayIndex;
        af[p,j,i] := Abstand(Rigg.rP[ooF0],Rigg.rP[ooC]);
      end;
      if yavAuslenkungC in YAchseSet then begin
        j := YAchseRecordList[yavAuslenkungC].ArrayIndex;
        if PunktOK then af[p,j,i] := Abstand(Rigg.rP[ooC],Rigg.rPe[ooC])
        else af[p,j,i] := 0;
      end;
      if yavDurchbiegungHD in YAchseSet then begin
        j := YAchseRecordList[yavDurchbiegungHD].ArrayIndex;
        af[p,j,i] := Rigg.hd;
      end;
      if yavRF00 in YAchseSet then begin
        j := YAchseRecordList[yavRF00].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[0]
        else af[p,j,i] := 0;
      end;
      if yavRF01 in YAchseSet then begin
        j := YAchseRecordList[yavRF01].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[1]
        else af[p,j,i] := 0;
      end;
      if yavRF03 in YAchseSet then begin
        j := YAchseRecordList[yavRF03].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[3]
        else af[p,j,i] := 0;
      end;
      if yavRF05 in YAchseSet then begin
        j := YAchseRecordList[yavRF05].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[5]
        else af[p,j,i] := 0;
      end;
      if yavRF10 in YAchseSet then begin
        j := YAchseRecordList[yavRF10].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[10]
        else af[p,j,i] := 0;
      end;
      if yavRF11 in YAchseSet then begin
        j := YAchseRecordList[yavRF11].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[11]
        else af[p,j,i] := 0;
      end;
      if yavRF13 in YAchseSet then begin
        j := YAchseRecordList[yavRF13].ArrayIndex;
        if PunktOK then af[p,j,i] := Rigg.rF[13]
        else af[p,j,i] := 0;
      end;

      Application.ProcessMessages;
      if ProgressDlg.Aborted then Break;
    end; {i-Schleife}
    if ProgressDlg.Aborted then begin
      Reset;
      Break;
    end;
  end; {p-Schleife}
...
begin
  ...
  YGap := Round((Ymax-Ymin)/10)+1; if YGap = 0 then YGap := 0.2;
  {Ymax-Ymin darf nicht Null sein - sonst Abbruch nach VBX-Exception!!!}
  if (Ymax-Ymin < 0.1) then begin Ymin := Ymin-0.1; Ymax := Ymax+0.1; end;
  {Xmax-Xmin darf nicht Null sein - sonst Abbruch nach VBX-Exception!!!}
  if (Xmax-Xmin < 0.1) then begin Xmin := Xmin-0.1; Ymax := Ymax+0.1; end;
...
var
  p, j: Integer;
begin
  if (YComboBox.ItemIndex < 0) and (YComboBox.ItemIndex > VNr-1) then begin
    FValid := False;
    YMinEdit.Text := 'YMin';
    YMaxEdit.Text := 'YMax';
    Draw;
...
procedure TChartForm.KurvenZahlSpinnerClick(Sender: TObject;
  Button: TUDBtnType);
begin
  if (tempSpinnerPosition = ParamCount) and (Button = btNext) then Exit;
  if (tempSpinnerPosition = 1) and (Button = btPrev) then Exit;
  if ShowGroup then ShowTogetherBtnClick(Sender);
end;

procedure TChartForm.ShowTogetherBtnClick(Sender: TObject);
begin
  if FStatus = [] then Exit;
...
begin
  if ParamNo > ParamCount then ParamNo := ParamCount;
  ...
  GroupKurvenZahl := p;
  for p := 0 to GroupKurvenZahl-1 do begin
    ...
    if max-min = 0 then
      for i := 0 to LineCount do bf[p,i] := temp
    else begin
...
var
  i, j, p: Integer;
begin
  if RggDocument.CalcTyp = ctQuerKraftBiegung then begin
...
var
  i: Integer;
begin
  with YAuswahlDlg do begin
    if not (DstList.ItemIndex = -1) then begin
...
begin
  result := '';
  for YAV := Low(TYAchseValue) to High(TYAchseValue) do
    if YAchseRecordList[YAV].ComboText = Text then begin
...
procedure TChartForm.PSpinnerChanging(Sender: TObject;
  var AllowChange: Boolean);
begin
  if ParamCount = 1 then AllowChange := False;
end;
...
begin
  with MemoLines do begin
    ...
    if SalingTyp <> stOhne then {case statement}
    ...
    {Y}
    if YAchseSet <> [] then begin
      Add('');
      Add('YAchse: Berechnete Kurven');
      for YAV := Low(TYAchseValue) to High(TYAchseValue) do
        if YAV in YAchseSet then Add('  ' + YAchseRecordList[YAV].ComboText);
    end;
    { Längen }
    xpName := GetTsbName(PComboText);
    Add('');
    Add('Rigg: Einstellwerte');
    with RiggModul.Rigg do begin
...
var
  i: Integer;
begin
  with XComboBox.Items do begin
    Clear;
    if SalingTyp = stFest then begin
      Add('Controller');
      Add('Vorstag');
      Add('Winkel');
      Add('Wante');
    ...
    if SalingTyp = stDrehbar then begin
      Add('Controller');
      Add('Vorstag');
      Add('Wante');
      Add('Wante oben');
    ...
    if SalingTyp = stOhne_2 then begin
      Add('Controller');
      Add('Vorstag');
      Add('Wante');
    end;
    if SalingTyp = stOhne then begin
      Add('Vorstag');
    end;
  end;
  XComboBox.ItemIndex := XComboBox.Items.IndexOf('Vorstag');
  for i := 0 to XComboBox.Items.Count-1 do
    if (XComboBox.Items[i] = FXTextClicked) then begin
...
var
  i: Integer;
begin
  with PComboBox.Items do begin
    Clear;
    Add('kein Parameter');
    if XComboBox.Text = 'Controller' then begin
      if SalingTyp = stFest then begin
        Add('Vorstag');
        Add('Winkel');
        Add('Wante');
    ...
    else if (XComboBox.Text = 'Vorstag') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Wante');
        Add('Wante oben');
    ...
    else if (XComboBox.Text = 'Winkel') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Wante');
        Add('Wante oben');
    ...
    else if (XComboBox.Text = 'Wante') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Vorstag');
        Add('Winkel');
    ...
    else if (XComboBox.Text = 'Wante oben') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Vorstag');
        Add('Winkel');
    ...
    else if (XComboBox.Text = 'Saling Höhe') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Vorstag');
        Add('Winkel');
    ...
    else if (XComboBox.Text = 'Saling Abstand') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Vorstag');
        Add('Winkel');
    ...
    else if (XComboBox.Text = 'Saling Länge') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Vorstag');
        Add('Winkel');
    ...
    else if (XComboBox.Text = 'Saling Winkel') then begin
      if SalingTyp = stFest then begin
        Add('Controller');
        Add('Vorstag');
        Add('Winkel');
  ...
  for i := 0 to PComboBox.Items.Count-1 do
    if (PComboBox.Items[i] = FPTextClicked) then begin
...
begin
  ...
  if (SalingTyp = stFest) and (name = xpSalingL) then begin
  ...
  end else if name = xpSalingW then begin
    SalingDreieck.CopyFromRigg(RiggModul.Rigg);
  ...
  try
    if BereichBtn.Down then begin
      Minimum := tempMin;
      Maximum := tempMax;
    end
    else if APBtn.Down then begin
      Minimum := tempMin;
      Maximum := tempMax;
      if (tempIst - APWidth) > tempMin then Minimum := tempIst - APWidth;
      if (tempIst + APWidth) < tempMax then Maximum := tempIst + APWidth;
    end
    else begin
      try
