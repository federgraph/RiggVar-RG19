...
begin
  ...
  for m := ooA0 to ooF0 do
    for n := x to z do
      Grid.Cells[Ord(n) + 1, Ord(m)] := Format('%4.0f', [FiP[m,n]]);
end;
...
procedure TOptionForm.GridSelectCell(Sender: TObject; Col, Row: Integer; var CanSelect: Boolean);
begin
  CanSelect := True;
  ...
  begin
    ...
    RumpfLabel.Caption := Format('Feld %s%s:',
      [TrimLeft(Grid.Cells[0,Row]),
       TrimLeft(Grid.Cells[Col,0])]);
    RumpfSpinEdit.Position := StrToInt(Grid.Cells[Col,Row]);
    RumpfEdit.Text := Format('%4d mm', [RumpfSpinEdit.Position]);
  end;
end;
...
var
  ...
  kk: TKoord;
begin
  oo := TRiggPoint(FRumpfCell.Y);
  kk := TKoord(FRumpfCell.x - 1);
