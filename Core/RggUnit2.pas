...
type
  ...
  private
    ...
    function GetKraftGraph: TKraftGraph;
    procedure SetKraftGraph(Value: TKraftGraph);
  ...
  public
    ...
    property KraftGraph: TKraftGraph read GetKraftGraph write SetKraftGraph;
  end;
...
constructor TMast.Create(AOwner: TComponent);
begin
  FKorrigiert := True;
...
procedure TMast.SetKraftGraph(Value: TKraftGraph);
begin
...
begin
  ...
  try
    ...
    with bmp.Canvas do begin
      ...
      Pen.Color := clGreen;
      MoveTo(0, 0);
      LineTo(Round(KoppelFaktor*10000*SalingAlpha*FZoomX),
             Round(10000*FZoomY));
...
begin
  ...
  tempControllerTyp := FControllerTyp;
  tempKorrigiert := FKorrigiert;

  for i := 0 to 150 do begin
    {FVerschoben}
    FKorrigiert := False;
    FControllerTyp := ctZugDruck;
    hd := i;
    CalcWKnick;
    if FMastOK or FShowAll then FVerschoben[i] := Round(-FC) {in N}
    else FVerschoben[i] := 0;
    ...
    hd := i;
    CalcWKnick;
    if FMastOK or FShowAll then FVerschobenKorrigiert[i] := Round(-FC) {in N}
    else FVerschobenKorrigiert[i] := 0;
  end;

  {hd und FKorrigiert restaurieren}
  hd := tempHd;
  FKorrigiert := tempKorrigiert;
  FControllerTyp := tempControllerTyp;
  CalcWKnick;
